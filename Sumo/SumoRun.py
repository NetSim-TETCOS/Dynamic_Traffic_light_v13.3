...
while step < 100000:
	...
	if ((vehicle_from_Netsim == "change traffic light")):
		...
		if traci.trafficlight.getPhase("0") == 2:
			...
			print("\n trafic light is changing from phase 2 to phase 3")
			traci.simulationStep()
		else:
			traci.trafficlight.setPhase("0", 2)
			print("\n trafic light is changing from phase 3 to phase 2")
			traci.simulationStep()
		...
		if len(k5)==0:						#No vehicle present, let the simulation continue
			traci.simulationStep()
		for i in k5:
			if i.lower() == vehicle_from_Netsim:
				k=1
				break;
		if k==1:
			if i == k5[0]:
